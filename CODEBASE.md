# nadavhames.com — how the site works

What the site is made of today, and why it is built this way. This file describes the current
state only: no history, no "used to be". The dated history is [PROJECT_LOG.md](PROJECT_LOG.md), and
...
AGENTS.md                  the rules for working here (CLAUDE.md just imports it)
CODEBASE.md                this file: how the site works today
PROJECT_LOG.md             dated history of every substantive change
README.md                  the short human-facing readme
```
...
with a pen: the masthead's as the page loads (after the printed clef), the footer's once it scrolls
into view, closing on the final barline. How long each takes depends on how many strokes the tune
needs: from about 20 seconds for Scotland the Brave's masthead to about 35 for the Radetzky
March's, the busiest.
...
- Radetzky March (Johann Strauss I, 1848), public domain everywhere. There's no ABC of it in the
  usual archives, so the melody comes from the MIDI of flutetunes.com's solo flute arrangement:
...
- **Phones show all four bars** on both staffs. Two things make room. The staff is engraved
  ...
  the mark beside it. Nothing touches at
  320px, the narrowest width the site supports; measure again there after adding a busy tune.
...
letters dataset has no "/", so it is drawn with a slanted barline stroke. Some of the writer's small letters are as tall as
capitals (their "a" reads as "A"), so the extractor scales those to the x-height, and spaces
...
arrangement on 8notes.com (read from its MIDI, in C, transposed up a tone): E then A7 under
...
- **The strokes are real handwriting.** Music comes from HOMUS (Handwritten Online Musical Symbols;
  ...
  (Llorens et al., LREC 2008; UCI repository, **CC BY 4.0 — attribution required**), letters,
  digits and "." by writer W14. Both are credited in `handwriting.ts`, each staff's `<desc>`,
  and here.
