...
function game() {
      ...
      if (document.getElementById(newid)) {
        if (document.getElementById(newid).style.backgroundColor == opponentColor) {
          ...
          if (i > 0 && document.getElementById(newid).style.backgroundColor != opponentColor) {
            if (
              document.getElementById(newid).style.backgroundColor != turn[0].style.backgroundColor
            ) {
  ...
  if (moves === undefined || moves.length == 0) {
    ...
    if (b > w) {
      text.innerHTML = "BLACK WINS! (" + b + "," + w + ")";
    }
    if (w > b) {
      text.innerHTML = "WHITE WINS! (" + w + "," + b + ")";
    }
    if (w == b) {
      text.innerHTML = "TIE! (" + w + "," + b + ")";
    }
  }
  ...
  function flipPieces(m) {
    ...
    function getAllIndexes(m) {
      ...
      for (n = 0; n < paths.length; n++) {
        if (paths[n] === m) indexes.push(n);
      }
      return indexes;
    }

    var indexes = getAllIndexes(m, paths);
    //console.log(indexes);
